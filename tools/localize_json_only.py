...
def collect_sources(value: Any) -> set[str]:
    ...
    def walk(v: Any) -> None:
        if isinstance(v, dict):
            uk = v.get("uk")
            if has_cyr(uk): out.add(uk)
            ex_uk = v.get("exampleUk")
            if has_cyr(ex_uk): out.add(ex_uk)
            for key, child in v.items():
                if key in LOCALIZED_KEYS and has_cyr(child): out.add(child)
                if key in SIBLING_KEYS and has_cyr(child): out.add(child)
                if key in LOCALIZED_LIST_KEYS and isinstance(child, list):
                    for item in child:
                        if has_cyr(item): out.add(item)
                walk(child)
...
def invalid(target: str, text: str) -> bool:
    if not text.strip() or WEIRD.search(text): return True
    if target == "en" and CYR.search(text): return True
    if target == "ru" and UK_ONLY.search(text): return True
    return False
...
def batch_translate(chunks: list[str], target: str) -> dict[str, str]:
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(primary); model.eval()
    out: dict[str, str] = {}; bad: list[str] = []; batch_size = 72
    with torch.inference_mode():
        for start in range(0, len(chunks), batch_size):
            ...
            for src, dst in zip(batch, decoded):
                dst = dst.strip()
                if invalid(target, dst): bad.append(src)
                else: out[src] = dst
            print(f"{target} primary {min(start+batch_size,len(chunks))}/{len(chunks)} fallback={len(bad)}", flush=True)
    del model, tok; gc.collect()

    if bad:
        tok = AutoTokenizer.from_pretrained("facebook/nllb-200-distilled-600M", src_lang="ukr_Cyrl")
        model = AutoModelForSeq2SeqLM.from_pretrained("facebook/nllb-200-distilled-600M"); model.eval()
        ...
        with torch.inference_mode():
            for start in range(0, len(bad), 32):
                ...
                for src, dst in zip(batch, decoded):
                    dst = dst.strip()
                    if invalid(target, dst): raise RuntimeError(f"invalid {target} translation: {src!r} -> {dst!r}")
                    out[src] = dst
...
def build_translator(sources: set[str]) -> Callable[[str, str], str]:
    segmented = {s: split_segments(s) for s in sources}
    fixed: dict[tuple[str, str], str] = {}
    for source in sources:
        for target in ("ru", "en"):
            value = fixed_translation(target, source)
            if value is not None: fixed[(target, source)] = value

    chunks = sorted({piece.strip() for source, parts in segmented.items() if ("ru", source) not in fixed or ("en", source) not in fixed for flag, piece in parts if flag and piece.strip()})
    ...
    def translate(target: str, source: str) -> str:
        ...
        if invalid(target, value): raise RuntimeError(f"invalid normalized {target}: {source!r} -> {value!r}")
        cache[key] = value
        return value
...
def audit(doc: dict[str, Any]) -> list[str]:
    ...
    def walk(v: Any, path: str) -> None:
        if isinstance(v, dict):
            ...
            if has_cyr(uk):
                for lang in ("ru", "en"):
                    if not isinstance(v.get(lang), str) or not v[lang].strip(): errors.append(f"{path}: missing {lang}")
            if has_cyr(v.get("exampleUk")):
                if not isinstance(v.get("exampleRu"), str): errors.append(f"{path}: missing exampleRu")
                if not isinstance(v.get("exampleEn"), str): errors.append(f"{path}: missing exampleEn")
            for key, child in v.items():
                if key in LOCALIZED_KEYS and has_cyr(child): errors.append(f"{path}.{key}: raw Cyrillic not localized")
                if key in LOCALIZED_LIST_KEYS and isinstance(child, list):
                    for i,item in enumerate(child):
                        if has_cyr(item): errors.append(f"{path}.{key}[{i}]: raw Cyrillic not localized")
    ...
    for lesson in doc.get("lessons", []):
        ...
        if loc.get("uiLanguages") != ["uk", "ru", "en"]: errors.append(f"{lesson.get('id')}: uiLanguages")
    return errors


def main() -> None:
    ap = argparse.ArgumentParser(); ap.add_argument("--level", choices=("a1", "a2"), required=True); args = ap.parse_args()
    expected = 75 if args.level == "a1" else 90
    paths = sorted(Path("lessons", args.level).glob(f"{args.level}-s*-l*.json"))
    if len(paths) != expected: raise SystemExit(f"expected {expected}, got {len(paths)}")
    docs: list[tuple[Path,dict[str,Any]]] = []; sources: set[str] = set()
    for path in paths:
        doc = json.loads(path.read_text(encoding="utf-8")); docs.append((path,doc)); sources |= collect_sources(doc)
    ...
    if total_errors:
        print("\n".join(total_errors[:100])); raise SystemExit(f"audit errors: {len(total_errors)}")

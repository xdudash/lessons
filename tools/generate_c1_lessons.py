...
def extract_lessons() -> list[dict[str, object]]:
    ...
    if len(lessons) != 90:
        raise RuntimeError(f"Expected 90 C1 lessons, found {len(lessons)}")
    return lessons
...
def make_words(lesson_id: str, lesson: dict[str, object]) -> list[dict[str, object]]:
    ...
    for idx, sk in enumerate(raw_words, 1):
        ...
        words.append({
            ...
            "level": "C1",
            "topic": title,
            "tags": [str(lesson["section_id"]), "c1", "ownership:new"],
            "ownership": "NEW",
        })
    return words
...
def make_exercises(lid: str, title: str, words: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    return [
        ...
        {"id": f"{lid}-e04", "lessonId": lid, "type": "dropdown_blank", "order": 4, "question": "Обери точну фразу для C1-речення.", "questionLocalized": l10n("Обери точну фразу для C1-речення.", "Выбери точную фразу для C1-предложения.", "Choose the precise phrase for a C1 sentence."), "wordIds": [w[3]["id"]], "sentenceParts": [{"text": "V tejto situácii je dôležité použiť "}, {"blankId": "blank1", "options": [w[3]["sk"], w[0]["sk"], w[1]["sk"]], "correct": w[3]["sk"]}, {"text": "."}]},
        ...
        {"id": f"{lid}-e06", "lessonId": lid, "type": "multiple_select", "order": 6, "question": "Обери C1-репліки з природним тоном.", "questionLocalized": l10n("Обери C1-репліки з природним тоном.", "Выбери C1-реплики с естественным тоном.", "Select C1 replies with a natural tone."), "wordIds": [w[0]["id"], w[5]["id"]], "options": [{"id": "a", "sk": "Rozumiem, ale potrebujem to presnejšie vysvetliť.", "correct": True}, {"id": "b", "sk": "To je úplne mimo a nemá zmysel hovoriť.", "correct": False}, {"id": "c", "sk": "Z môjho pohľadu je dôležitý aj kontext.", "correct": True}]},
        ...
        {"id": f"{lid}-e12", "lessonId": lid, "type": "sentence_order", "order": 12, "question": "Віднови логічний порядок C1-відповіді.", "questionLocalized": l10n("Віднови логічний порядок C1-відповіді.", "Восстанови логический порядок C1-ответа.", "Restore the logical order of a C1 answer."), "wordIds": [w[0]["id"], w[1]["id"]], "items": ["Najprv treba pomenovať kontext.", "Potom možno doplniť presnejší výraz.", "Napokon z toho vyplýva konkrétny záver."], "correctOrder": ["Najprv treba pomenovať kontext.", "Potom možno doplniť presnejší výraz.", "Napokon z toho vyplýva konkrétny záver."]},
...
def make_lesson(lesson: dict[str, object], next_id: str | None) -> dict[str, object]:
    ...
    lid = f"c1-s{section:02d}-l{n:03d}"
    lesson["section_id"] = f"c1_s{section:02d}"
    title = str(lesson["title_sk"])
    ...
    desc = loc(
        f"Precvičíš si tému {title} v komplexnej komunikácii na úrovni C1.",
        f"Тренуємо тему «{title}»: аргументовано, точно й у реальному контексті.",
        what_ru,
        f"Practice the C1 topic “{title}” in complex communication. Section: {section_title}.",
    )
    exs = make_exercises(lid, title, words)
    return {"lessons": [{
        "id": lid,
        "sectionId": f"c1_s{section:02d}",
        "level": "C1",
        ...
        "startScreen": {
            ...
            "outcomes": [
                ...
                l10n("використати нові C1-фрази в контексті", "использовать новые C1-фразы в контексте", "use new C1 phrases in context"),
        ...
        "theoryScreens": [
            {"id": "t1", "screenType": "theory", "order": 1, "title": l10n("Фокус уроку", "Фокус урока", "Lesson focus"), "body": l10n(desc["uk"], desc["ru"], desc["en"]), "examples": [{"sk": words[0]["exampleSk"], "translation": l10n(words[0]["exampleUk"], words[0]["exampleRu"], words[0]["exampleEn"])}, {"sk": words[1]["exampleSk"], "translation": l10n(words[1]["exampleUk"], words[1]["exampleRu"], words[1]["exampleEn"])}], "shortRule": l10n("На C1 важливо не просто відповісти, а показати причину, умову, наслідок або нюанс.", "На C1 важно не просто ответить, а показать причину, условие, последствие или нюанс.", "At C1, show a reason, condition, consequence or nuance."), "button": "Далі"},
            ...
            {"id": "t3", "screenType": "theory", "order": 3, "title": l10n("Перенесення в життя", "Перенос в жизнь", "Real-life transfer"), "body": l10n("У фіналі потрібно вибрати репліку, яка реально рухає ситуацію вперед.", "В финале нужно выбрать реплику, которая реально двигает ситуацию дальше.", "In the final task, choose the reply that moves the situation forward."), "examples": [{"sk": "Navrhujem, aby sme si najprv ujasnili priority.", "translation": l10n("Пропоную спочатку прояснити пріоритети.", "Предлагаю сначала прояснить приоритеты.", "I suggest clarifying priorities first.")}], "shortRule": l10n("C1 = самостійна, зв'язна й доречна комунікація.", "C1 = самостоятельная, связная и уместная коммуникация.", "C1 means independent, connected and appropriate communication."), "button": "До слів"},
        ],
        "wordsScreen": {"screenType": "lesson_words", "title": l10n("Цільові слова й фрази", "Целевые слова и фразы", "Target words and phrases"), "description": l10n("Ці одиниці взяті з затвердженого C1-плану.", "Эти единицы взяты из утвержденного C1-плана.", "These items come from the approved C1 plan."), "items": [{"wordId": x["id"], "sk": x["sk"], "uk": x["uk"], "ru": x["ru"], "en": x["en"], "pronunciationUk": x["pronunciationUk"], "exampleSk": x["exampleSk"], "exampleUk": x["exampleUk"], "exampleRu": x["exampleRu"], "exampleEn": x["exampleEn"]} for x in words], "button": "До практики"},
        "words": words,
        "exercises": exs,
        "finalSituation": {
            "id": "final-situation",
            ...
            "description": l10n("Обери репліки, які звучать природно, точно й доречно на рівні C1.", "Выбери реплики, которые звучат естественно, точно и уместно на уровне C1.", "Choose replies that sound natural, precise and appropriate at C1 level."),
            "steps": [
...
def main() -> int:
    ...
    for lesson, next_lesson in zip(lessons, lessons[1:] + [None]):
        next_id = None if next_lesson is None else f"c1-s{int(next_lesson['section']):02d}-l{int(next_lesson['number']):03d}"
        doc = make_lesson(lesson, next_id)
        path = OUT / doc["lessons"][0]["id"]
        path = path.with_suffix(".json")
        path.write_text(json.dumps(doc, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (OUT / "README.md").write_text("# SlovakGo C1 Lessons\n\n90 active C1 lessons generated from `C1/LESSON_PLAN.md`.\n", encoding="utf-8")
    print(f"Generated {len(lessons)} C1 lessons in {OUT}")
    return 0

...
def assert_learner_langs(obj, path):
    assert isinstance(obj, dict), f"{path}: not object"
    missing = LEARNER_LANGS - set(obj)
    assert not missing, f"{path}: missing learner languages {sorted(missing)}"
    for lang in LEARNER_LANGS:
        assert isinstance(obj[lang], str) and obj[lang].strip(), f"{path}: empty {lang}"
...
def check(path: Path):
    ...
    lid = lesson["id"]
    m = re.fullmatch(r"c2-s(\d{2})-l(\d{3})", lid)
    assert m, f"{path.name}: bad id {lid}"
    section = int(m.group(1))
    order = int(m.group(2))
    assert path.stem == lid, f"{path.name}: filename != id"
    assert lesson["sectionId"] == f"c2_s{section:02d}", f"{path.name}: sectionId"
    assert lesson["level"] == "C2", f"{path.name}: level"
    ...
    assert len(lesson["theoryScreens"]) >= 2, f"{path.name}: theoryScreens"
    words = lesson["words"]
    ...
    for word in words:
        ...
        assert word["level"] == "C2", f"{path.name}: word level"

    exercises = lesson["exercises"]
    assert len(exercises) >= 14, f"{path.name}: exercises"
    assert lesson["startScreen"]["exercisesCount"] == len(exercises), f"{path.name}: start exercise count"
    ...
    assert not any("b1" in s or "b2" in s for s in all_text), f"{path.name}: lower-level leak"
    bad_fragments = ("вираз «", "выражение «", "the expression “", "словацьке слово/вираз", "словацкое слово/выражение", "praktická fráza")
    assert not any(fragment in s for s in all_text for fragment in bad_fragments), f"{path.name}: generic translation fallback leak"
    return lesson
...
def main() -> int:
    ...
    print("QA PASS: 70 C2 lessons, all language fields and nextLesson chain verified.")
    return 0

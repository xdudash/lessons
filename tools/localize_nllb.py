...
def split_quotes(parts: list[tuple[bool, str]]) -> list[tuple[bool, str]]:
    ...
    for flag, piece in parts:
        if not flag:
            out.append((flag, piece)); continue
        pos = 0
        for m in QUOTE_RE.finditer(piece):
            before = piece[pos:m.start()]
            if before: out.append((bool(base.CYR.search(before)), before))
            out.append((False, m.group(1)))
            out.append((True, m.group(2)))
            out.append((False, m.group(3)))
            pos = m.end()
        tail = piece[pos:]
        if tail: out.append((bool(base.CYR.search(tail)), tail))
...
def primary_translate(chunks: list[str], target: str) -> tuple[dict[str, str], list[str]]:
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(PRIMARY[target]); model.eval()
...
def nllb_pass(chunks: list[str], target: str, src_lang: str) -> tuple[dict[str, str], list[str]]:
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(NLLB); model.eval()
    forced_bos = tokenizer.convert_tokens_to_ids(TARGET_CODES[target])
    out: dict[str, str] = {}; bad: list[str] = []; batch_size = 32
    with torch.inference_mode():
        for start in range(0, len(chunks), batch_size):
            batch = chunks[start:start + batch_size]
            inputs = tokenizer(batch, return_tensors="pt", padding=True, truncation=True, max_length=384)
...
def translate_sources(strings: list[str], target: str) -> dict[str, str]:
    segmented = {s: split_quotes(base.split_translation_segments(s)) for s in strings}
    ...
    for source, parts in segmented.items():
        ...
        for translate, piece in parts:
            ...
            else: built.append(piece)
        value = "".join(built)
        literal_tokens = [p for flag, p in parts if not flag and base.LATIN_TOKEN.fullmatch(p)]
        for token in literal_tokens:
            if token not in value: raise RuntimeError(f"lost Slovak/Latin token {token!r}: {source!r} -> {value!r}")
        result[source] = value
    return result

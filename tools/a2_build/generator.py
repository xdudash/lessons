...
3:("Минулий час у розповіді","У минулому важливо одразу показати часову рамку: včera, minulý víkend, minulý týždeň.","Зв’язуй події маркерами porom/neskôr/nakoniec і не змішуй часові плани без потреби."),
...
def example_for(sk:str, uk:str)->tuple[str,str]:
    ...
    # Already clause-like / multiword functional chunks: use as a complete short utterance.
    if ' ' in low:
        if low.endswith(' sa'):
            base=sk[:-3]
    ...
    if low.endswith(('ť','ť?')):
        return (f'Chcem {sk}.', f'Я хочу {uk}.')
    # likely adjective/comparative
    if low.endswith(('ý','á','í')):

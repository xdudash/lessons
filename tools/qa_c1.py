...
def check(path: Path):
    ...
    lid = lesson["id"]
    m = re.fullmatch(r"c1-s(\d{2})-l(\d{3})", lid)
    assert m, f"{path.name}: bad id {lid}"
    section = int(m.group(1))
    order = int(m.group(2))
    assert path.stem == lid, f"{path.name}: filename != id"
    assert lesson["sectionId"] == f"c1_s{section:02d}", f"{path.name}: sectionId"
    assert lesson["level"] == "C1", f"{path.name}: level"
    ...
    for word in words:
        ...
        assert word["level"] == "C1", f"{path.name}: word level"
...
def main() -> int:
    ...
    print("QA PASS: 90 C1 lessons, all language fields and nextLesson chain verified.")
    return 0

...
UNRESOLVED:
- Exact item-level vocabulary selection for each expansion lesson.
...
NEXT TASK:
- Generate a small reviewed A1 pilot from the target registry (not mass generation), beginning with S01 expansion slots. For each pilot lesson, select exact vocabulary against the durable ownership data, build JSON using the established lesson schema, then run structural, duplication, answer-validity, CEFR/progression and naturalness QA before committing.
